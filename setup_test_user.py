...
def setup_test_user():
    ...
    print("\n📊 User Summary:")
    print(
        f"  Admin User: {test_user.username} (Role: {test_user.role.name if test_user.role else 'None'})"
    )
    print(
        f"  Manager User: {manager_user.username} (Role: {manager_user.role.name if manager_user.role else 'None'})"
    )
