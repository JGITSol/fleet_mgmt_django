...
class ComprehensiveUITester:
    ...
    def test_api_endpoints(self):
        ...
        jwt_token = None
        try:
            auth_response = self.session.post(f"{self.base_url}/api/auth/login/", json={
                'username': 'testuser',
                'password': 'testpass123'
            })
...
"""
    ...
    def run_comprehensive_test(self):
        ...
        url_success_rate = sum(1 for test in results['url_tests'] if test.get('success', False)) / len(results['url_tests']) * 100 if results['url_tests'] else 0
        api_success_rate = sum(1 for test in results['api_tests'] if test.get('success', False)) / len(results['api_tests']) * 100 if results['api_tests'] else 0
        ...
        self.log(f"Screenshots: {'✓ CAPTURED' if results['screenshots_captured'] else '✗ FAILED'}")
        self.log(f"Django Tests: {'✓ PASS' if results['django_tests_passed'] else '✗ FAIL'}")
        
        overall_success = (
            url_success_rate > 80 and 
            api_success_rate > 80 and 
            results['auth_test']
        )
        
        self.log(f"Overall Status: {'✓ SUCCESS' if overall_success else '✗ ISSUES DETECTED'}")
