...
class ComprehensiveUITester:
    ...
    def test_authentication_flow(self):
        ...
        try:
            # Get login page first to get CSRF cookie
            ...
            if csrftoken:
                ...
                self.log("⚠ Could not find CSRF token in cookies")
            ...
            if response.status_code == 200 or response.status_code == 302:
                # Check if we are actually logged in (redirected or success)
                # If we are still on login page with 200, it might mean failure
                if "Log In" in response.text and "password" in response.text:
                     self.log(f"✗ Login test failed: Still on login page")
                     return False
