"""
Improved comprehensive translation generator
Handles both empty and existing translations, adds more strings
"""
...
# Comprehensive translation dictionary with ALL common strings
TRANSLATIONS = {
...
def update_po_file_improved(po_file_path, language_code):
    """Update a .po file with translations, handling both empty and existing translations"""
    ...
    # Pattern to match msgid/msgstr pairs (empty or existing)
    pattern = r'msgid "([^"]+)"\nmsgstr "([^"]*)"'
    ...
    def replace_translation(match):
        ...
        # Only update if we have a translation for this string
        if msgid in TRANSLATIONS and language_code in TRANSLATIONS[msgid]:
            new_translation = TRANSLATIONS[msgid][language_code]
            # Update if empty OR if different from our translation
            if not existing_msgstr or existing_msgstr != new_translation:
                updated_count += 1
...
def main():
    ...
    print("=" * 60)
    print("Generating Comprehensive Translations (Improved)")
    print("=" * 60)
    ...
    for lang_code, lang_name in languages.items():
        ...
        count = update_po_file_improved(str(po_file), lang_code)
    ...
    print("2. Restart the Django server (or kill Python processes)")
