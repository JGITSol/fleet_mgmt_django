...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if environment != 'development':
            self.stdout.write(
                self.style.ERROR(f"❌ ERROR: This command should only be run in development mode!")
            )
        ...
        self.stdout.write(f"   - Role: Admin (full access)")
        self.stdout.write("")
        self.stdout.write(f"   Regular User:")
        self.stdout.write(f"   - Username: {user_username}")
        self.stdout.write(f"   - Password: {user_password}")
        self.stdout.write(f"   - Email: {user_email}")
        self.stdout.write(f"   - Role: Driver (limited access)")
        self.stdout.write("\n🌐 Access URLs:")
