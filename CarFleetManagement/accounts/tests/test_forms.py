...
class CustomUserCreationFormTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data."""
        # Use get_or_create to avoid UNIQUE collisions across test runs
        self.admin_role, _ = UserRole.objects.get_or_create(
            name=UserRole.ADMIN, defaults={"description": "Administrator role"}
        )
        self.driver_role, _ = UserRole.objects.get_or_create(
            name=UserRole.DRIVER, defaults={"description": "Driver role"}
        )
    ...
    def test_form_save(self):
        ...
        self.assertTrue(form.is_valid())
        user = form.save()
        self.assertEqual(user.username, "testuser_forms")
    ...
    def test_form_fields_included(self):
        ...
        expected_fields = ["username", "email", "role", "phone_number", "emergency_contact", "password1", "password2"]
