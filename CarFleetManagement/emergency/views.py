...
class EmergencyIncidentUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return self.request.user.is_staff or (
            hasattr(self.request.user, "role") and self.request.user.role.name in ["ADMIN", "EMERGENCY_STAFF"]
        )
...
class EmergencyIncidentDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def test_func(self):
        ...
        return self.request.user.is_staff or (
            hasattr(self.request.user, "role") and self.request.user.role.name == "ADMIN"
        )
