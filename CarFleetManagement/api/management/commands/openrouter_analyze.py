...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if subcommand == 'analyze':
            ...
            self.batch_analyze(input_file, output, filters, prompt)
        elif subcommand == 'report':
            self.generate_report(screenshot, output)
        else:
    ...
    def batch_analyze(self, input_file, output_file=None, filters=None, prompt=None):
        ...
        for screenshot in screenshots:
            if not os.path.exists(screenshot):
                self.stdout.write(self.style.WARNING(f'Screenshot not found: {screenshot}'))
                continue
            result = client.analyze_screenshot(screenshot, prompt=prompt)
            results[os.path.basename(screenshot)] = result
            self.stdout.write(self.style.SUCCESS(f'Analyzed: {screenshot}'))
        if output_file:
            with open(output_file, 'w') as f:
                json.dump(results, f)
